...
if (isset($_GET['api']) && $_GET['api'] === 'true') {
    ...
    switch ($apiAction) {
        case 'get_paths':
            ...
            echo json_encode(getValidPaths($basePath));
            break;
        case 'upload_file':
            echo json_encode(uploadFileViaApi());
            break;
        case 'delete_file':
...
class FileManager {
    ...
    private function deleteItem() {
        ...
        $path = $this->currentDir . '/' . $name;
        if ($path === __FILE__) {
            $this->addMessage('This file cannot be deleted.', 'danger');
            return;
        }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { background: var(--bg-body); color: var(--text-primary); font-family: 'Inter', system-ui, sans-serif; padding: 12px; min-height: 100vh; }
        .fm-container { max-width: 1600px; margin: 0 auto; background: var(--bg-surface); border-radius: var(--radius); box-shadow: var(--shadow); border: 1px solid var(--border-color); overflow: hidden; }
        .fm-sidebar { background: var(--bg-elevated); padding: 16px; border-right: 1px solid var(--border-color); transition: all 0.3s ease; height: 100%; overflow-y: auto; }
        .fm-sidebar .nav-btn { display: block; width: 100%; padding: 12px 14px; margin-bottom: 6px; border-radius: 8px; border: none; background: transparent; color: var(--text-secondary); text-align: left; transition: var(--transition); font-weight: 500; font-size: 15px; }
        .fm-sidebar .nav-btn i { margin-right: 12px; font-size: 1.2rem; width: 1.4rem; text-align: center; }
        .fm-sidebar .nav-btn:hover { background: var(--bg-hover); color: var(--text-primary); }
        .fm-sidebar .nav-btn.active { background: var(--accent); color: #fff; }
        .fm-sidebar hr { border-color: var(--border-color); margin: 14px 0; }
        .fm-sidebar .folder-list a { display: flex; align-items: center; padding: 10px 12px; border-radius: 6px; color: var(--text-secondary); text-decoration: none; transition: var(--transition); font-size: 15px; margin-bottom: 2px; }
        .fm-sidebar .folder-list a i { margin-right: 12px; color: var(--warning); font-size: 1.2rem; width: 1.4rem; text-align: center; }
        .fm-sidebar .folder-list a:hover { background: var(--bg-hover); color: var(--text-primary); }
        .fm-sidebar .folder-list .empty { color: var(--text-muted); font-size: 14px; padding: 10px 12px; }
        .sidebar-title { font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); margin-bottom: 10px; }
        .sidebar-toggle { display: none; background: transparent; border: 1px solid var(--border-color); color: var(--text-primary); padding: 6px 12px; border-radius: 8px; font-size: 1.5rem; line-height: 1; cursor: pointer; transition: var(--transition); }
        .sidebar-toggle:hover { background: var(--bg-hover); }
        .fm-content { padding: 16px 20px; background: var(--bg-body); }
        .fm-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px; }
        .fm-header h5 { font-weight: 600; font-size: 1.1rem; margin: 0; color: var(--text-primary); display: flex; align-items: center; gap: 8px; }
        .fm-header h5 i { color: var(--accent); font-size: 1.3rem; }
        .fm-header .btn-logout { background: transparent; border: 1px solid var(--border-color); color: var(--text-secondary); padding: 8px 16px; border-radius: 8px; transition: var(--transition); font-size: 0.9rem; }
        .fm-header .btn-logout:hover { background: var(--danger); border-color: var(--danger); color: #fff; }
        .fm-breadcrumb { background: var(--bg-elevated); padding: 10px 14px; border-radius: 8px; margin-bottom: 18px; border: 1px solid var(--border-color); display: flex; align-items: center; flex-wrap: wrap; gap: 4px; font-size: 14px; color: var(--text-secondary); word-break: break-all; }
        .fm-breadcrumb a { color: var(--accent); text-decoration: none; }
        .fm-breadcrumb a:hover { text-decoration: underline; }
        .fm-alert { padding: 12px 16px; border-radius: 8px; margin-bottom: 14px; border-left: 5px solid; font-size: 14px; background: var(--bg-elevated); }
        ...
        .fm-toolbar { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 20px; background: var(--bg-elevated); padding: 14px 16px; border-radius: var(--radius); border: 1px solid var(--border-color); align-items: center; }
        .fm-toolbar .form-control { background: var(--bg-input); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 8px; padding: 10px 14px; font-size: 15px; transition: var(--transition); width: 100%; min-width: 120px; }
        .fm-toolbar .form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(91,124,250,0.15); }
        .fm-toolbar .form-control::placeholder { color: var(--text-muted); }
        .fm-toolbar .btn { border-radius: 8px; padding: 10px 16px; font-weight: 500; font-size: 14px; border: none; transition: var(--transition); white-space: nowrap; }
        ...
        .fm-table { background: var(--bg-elevated); border-radius: var(--radius); overflow-x: auto; border: 1px solid var(--border-color); }
        .fm-table table { width: 100%; border-collapse: collapse; color: var(--text-primary); font-size: 14px; min-width: 600px; }
        .fm-table thead { background: var(--bg-hover); border-bottom: 1px solid var(--border-color); }
        .fm-table th { padding: 12px 12px; font-weight: 600; color: var(--text-secondary); text-transform: uppercase; font-size: 12px; letter-spacing: 0.3px; text-align: left; }
        .fm-table td { padding: 12px 12px; border-bottom: 1px solid var(--border-color); vertical-align: middle; }
        ...
        .fm-table .file-name { font-weight: 500; color: var(--text-primary); text-decoration: none; display: inline-flex; align-items: center; }
        ...
        .fm-table .actions { display: flex; gap: 6px; justify-content: flex-end; flex-wrap: wrap; }
        .fm-editor { background: var(--bg-elevated); border-radius: var(--radius); border: 1px solid var(--border-color); overflow: hidden; }
        .fm-editor .editor-header { background: var(--bg-hover); padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); flex-wrap: wrap; gap: 8px; }
        ...
        .fm-editor textarea { width: 100%; min-height: 400px; background: #0d111a; color: #e9edf5; border: none; padding: 16px; font-family: 'Fira Code', 'JetBrains Mono', monospace; font-size: 14px; line-height: 1.7; resize: vertical; outline: none; }
        ...
        @media (max-width: 768px) {
            ...
            .fm-sidebar { display: none; border-right: none; border-bottom: 1px solid var(--border-color); padding: 12px; max-height: 70vh; overflow-y: auto; }
            .fm-sidebar.show { display: block; }
            .sidebar-toggle { display: inline-block; }
            .fm-header h5 { font-size: 1rem; }
            .fm-toolbar { flex-direction: column; align-items: stretch; }
            .fm-toolbar .d-flex { flex-direction: column; width: 100%; }
            .fm-toolbar .form-control { width: 100% !important; max-width: 100% !important; min-width: unset; }
            .fm-toolbar .btn { width: 100%; justify-content: center; }
            .fm-toolbar .go-to-form { flex-direction: row; width: 100%; }
            .fm-toolbar .go-to-form .form-control { flex: 1; }
            .fm-toolbar .go-to-form .btn { width: auto; flex-shrink: 0; }
            .fm-breadcrumb { font-size: 13px; padding: 8px 12px; }
            .fm-table table { font-size: 13px; min-width: 480px; }
            .fm-table th, .fm-table td { padding: 10px 8px; }
            .fm-table .actions .btn-sm { padding: 6px 10px; font-size: 12px; }
            .fm-editor textarea { min-height: 300px; font-size: 13px; padding: 12px; }
            .fm-header .btn-logout { padding: 6px 12px; font-size: 0.85rem; }
        }
        @media (max-width: 480px) {
            .fm-table table { min-width: 360px; font-size: 12px; }
